...
class SmarPod_functions:
    ...
    def set_speed_and_frequency_and_pivot_point_and_coordinate_system(self):
        ...
        smarpod.Set_ui(self.handle, smarpod.Property.PIVOT_MODE, smarpod.PivotMode.RELATIVE)
        smarpod.SetPivot(self.handle,[10e-3,0,0])

        coordinate_sys = Pose(0.,0.,0., 1.5,0.,0.)
        smarpod.SetCoordinateSystem(self.handle, coordinate_sys)
        logging.info("setting coordinate system to %s" % self.pose_to_str(coordinate_sys))
    ...
    def set_up(self, model, locator):
        try:
            ...
            frequency = 8000
            smarpod.Set_ui(self.handle, smarpod.Property.FREF_AND_CAL_FREQUENCY, frequency)
            ...
            if not smarpod.IsReferenced(self.handle):
                smarpod.Set_ui(self.handle, smarpod.Property.FREF_METHOD, smarpod.DEFAULT)
            ...
            while (move_status == smarpod.MoveStatus.CALIBRATING or move_status == smarpod.MoveStatus.REFERENCING):
    ...
    def moving(self):
        pose_sequence = self.load_position_data()
...
handle = smarpod_object.set_up(model, locator)

smarpod_object.moving()
